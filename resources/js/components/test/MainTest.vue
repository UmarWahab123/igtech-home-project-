...
<div id="main-wrapper" class="main-container 11"   v-if="this.loader==0">
   ...
   <div class="test-wrapper">
      ...
      <div id="questionboxB" class="container"  v-if="this.showSection==1"> 
           <!-- Type B -->
                  <div class="questionBwrapper" v-if="this.stage==3 && this.status==1">
                        <div class="subwrapper">
                       <div class="questionB-back">
                     ...
                     <img v-on:click="previousquestion" class="leftarrow pull-right" src="/test/public/svg/arrow-left.svg">
                           ...
                           <div class="back-text"  v-on:click="previousquestion">Back</div>
                        </div>
                        </div>


                        <div class="typeB-container">
                           <div class="typebheadline optheading">
                              <p class="mb-0">{{translations.questionB_headline}}</p>
                              <p>{{translations.questionB_text}}</p>
                           </div>
                              <div class="questionb-wrapper">
                                 <div class="typeb-radio">
                                     <svg class="svg-circle" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" viewBox="0 0 60 60" preserveAspectRatio="none">
                            <circle class="circle" cx="30" cy="30" r="28" id="green-halo" fill="none" stroke="#d4b6a6" stroke-width="3" stroke-dasharray="0,200" transform="" />
                            </svg>
                              <div v-on:click="sendresponse" :id="this.fields.statement_a_pid"  class="faux-radio lrg" data-val="3"></div>
                                 </div>
                             <div class="questionb-text qb1">
                           {{questiondata.statement_a_text}}
                           </div>
                              </div>
                               <div class="questionb-wrapper mb-0">
                                 <div class="typeb-radio">
                                       <svg class="svg-circle" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" viewBox="0 0 60 60" preserveAspectRatio="none">
                            <circle class="circle" cx="30" cy="30" r="28" id="green-halo" fill="none" stroke="#d4b6a6" stroke-width="3" stroke-dasharray="0,200" transform="" />
                            </svg>
    ...
    <div id="questionbox" class="container-fluid"  v-if="this.showSection==1"> 
             <!-- Type A -->
                  <div class="" v-if="(this.stage==1 || this.stage==2)  && this.status==1">
                     <div class="questionAdiv">
                        <div class="back-button">
                           <div class="backwrapper" v-if="this.previous==1">
                     <div class="previcon">
                     <img   v-on:click="previousquestion" class="leftarrow pull-right" src="/test/public/svg/arrow-left.svg">
                           </div>
                           <div class="back-text"  v-on:click="previousquestion">Back</div>
                     ...
                     <div class="row justify-content-center radio-row">
                        ...
                        <div class="col-md-2 col-sm-3 col-lg-2 col-xl-2 col-3 pl-0 pr-0 ml-0 mr-0">
                           <div class="form-group m-form__group mb-4">
                            <svg class="svg-circle" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" viewBox="0 0 60 60" preserveAspectRatio="none">
                            <circle class="circle" cx="30" cy="30" r="28" id="green-halo" fill="none" stroke="#d4b6a6" stroke-width="3" stroke-dasharray="0,500" transform="" />
                            </svg>
                              <div  v-on:click="sendresponse" data-val="0" class="faux-radio lrg"></div>
                           </div>
                           <div class="optheading">
                              <p class="mb-0">{{translations.strongly_disagree1}}</p>
                               </div>
                        </div>
                        <div class="col-md-2 col-sm-3 col-lg-2 col-xl-2 col-3 pl-0 pr-0 ml-0 mr-0">
                           <div class="form-group m-form__group mb-4">
                            ...
                            <div class="optheading">
                              <p class="mb-0">{{translations.slightly_disagree1}}</p>
                               </div>
                        </div>
                        <div class="col-md-2 col-sm-3 col-lg-2 col-xl-2 col-3 pl-0 pr-0 ml-0 mr-0">
                           <div class="form-group m-form__group mb-4">
                            ...
                            <circle class="circle" cx="30" cy="30" r="28" id="green-halo" fill="none" stroke="#d4b6a6" stroke-width="3" stroke-dasharray="0,500" transform="" />
                             ...
                             <div class="optheading">
                              <p class="mb-0">{{translations.slightly_agree1}}</p>
                               </div>
                        </div>
                        <div class="col-md-2 col-sm-3 col-lg-2 col-xl-2 col-3 pl-0 pr-0 ml-0 mr-0">
                           <div class="form-group m-form__group mb-4">
                           ...
                           <div class="optheading">
                              <p class="mb-0">{{translations.strongly_agree1}}</p>
                               </div>
                        </div>
                     </div>
                  </div>
   ...
   Vue.use(VueSimpleAlert);              
    export default {
        ...
        mounted() {
         ...
         //   window.onbeforeunload = function() { return "Are you sure you want to go back? When you leave you will loose your progress so far and have to restart."; };
            ...
            test(response) {
             ...
             sendresponse: function (event) {
                ...
                return false;
                  ...
                  parent = event.currentTarget.parentNode.querySelector('.svg-circle');
                  var child=parent.children;
                  var circle=child[0];
                  var backclass = document.getElementsByClassName('faux-radio')[0];
                  var backclass = event.currentTarget;
                  backclass.style.backgroundColor ='#d4b6a6';
                  backclass.style.borderColor ='#d4b6a6';
                  var interval = 10;
                  var angle = 0;
                  ...
                  this.PreviousData(); //assigning values to previous array
        ...
        QuestionResponse: function (event,ans,id) {
            ...
            if(response.data.status==1){
                ...
                let responsedata=response.data.response;
                 document.getElementById("main-wrapper").classList.remove("myreadonly");
               if(responsedata.type=="A"){
                  document.querySelector('.question').innerHTML='';
                 document.querySelector('.question').innerHTML=responsedata.question;;
               }
               console.log('qusetion=',responsedata.question);
               ...
               //  console.log('asfd',this.questiondata.statement_a_text);
                ...
                this.fields.stage=response.data.stage;
                 ...
                 document.querySelector('.progress-text').style.marginLeft=margin + "%";
                }
                else{
            ...
            else if(response.data.status==0){ //No Data
                this.status=response.data.status;
                this.stage=response.data.stage;
            } 
            else if(response.data.status==-1){ //same response
                this.showSame=1;
                this.showSection=0;
                 document.querySelector('.btnclose').style.display='none';
                 document.querySelector('.main-container').classList.add("blur");
            } 
            else if(response.data.status==4){ //winner declare
